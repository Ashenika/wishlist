...
/*global jQuery */
/*global _ */
/*global Backbone */// Generic Namespace for our App
var App = {Collection: {}, Model: {}, View: {}};
App.Model.Item = Backbone.Model.extend({
    defaults: {price: .99, quantity: 0, total: 0}, total: function () {
        var e = this.get("price") * this.get("quantity");
        this.set("total", e);
        return e
    }, quanity: function (e) {
        var t = this.get("quantity");
        this.set("quantity", e === "increase" ? ++t : --t)
    }
});
...
App.View.ItemList = Backbone.View.extend({
    el: "#default-item-list", initialize: function () {
        this.render()
    }, render: function () {
        this.collection.each(function (e) {
            var t = new App.View.Item({model: e});
...
App.View.ShoppingCartItemView = Backbone.View.extend({
    ...
    events: {"click .name": "remove", "click .quantity": "manageQuantity"},
    initialize: function () {
        this.render();
    ...
    manageQuantity: function (e) {
        var t = $(e.target).data("type");
        this.model.get("quantity") === 1 && t === "decrease" ? this.remove() : this.model.quanity(t)
    },
    remove: function () {
...
App.View.ShoppingCart = Backbone.View.extend({
    ...
    initialize: function () {
        ...
        this.defaultMessage();
        this.collection.on("add remove change:quantity", function (e) {
            this.updateTotal();
            this.collection.length === 0 && this.defaultMessage()
        }, this)
    },
    ...
    updateTotal: function () {
        ...
        this.basketTotal.html(e);
        this.total.html(this.collection.subtotal())
    },
    render: function () {
        this.$el.html("");
        this.collection.each(function (e) {
            var t = new App.View.ShoppingCartItemView({model: e});
            this.$el.append(t.render().el)
        }, this)
    }
});
var defaultItems = [{title: "Bacon", price: 2.99}, {title: "Cabbage"}, {title: "Spinnach", price: 1.4}, {
    title: "Salt",
    price: .88
}, {title: "Bread"}, {title: "Butter", price: 1.99}];
...
collection4.fetch({
    success: function (collection4, response) {
        ...
        collection4.each(function (items, index, all) {
            ...
            data.push({title: items.get("title"),price:items.get("price")});
        });
    }
});
...
App.cartItems.on("add", function (e) {
    e.set("quantity", 1)
});

console.log(data);

for (var i in data) App.items.add(new App.Model.Item(data[i]));
